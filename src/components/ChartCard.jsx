...
export function ChartCard({ stocks, eurRate }) {
  ...
  // Marker acquisti nel range visibile
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 11, color: isPositive ? "#5EC98A" : "#E87040" }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", padding: "8px 0 0", borderTop: "1px solid #0f1117", marginTop: 8, alignItems: "center" }}>
        {showBenchmark && (() => {
          const spyFirst = chartData.find(p => p.spyPct != null)?.spyPct;
          const spyLast  = [...chartData].reverse().find(p => p.spyPct != null)?.spyPct;
          if (spyFirst == null || spyLast == null) return null;
          const diff = spyLast - spyFirst;
          return (
            <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "#444" }}>
              ...
              <span style={{ color: diff >= 0 ? "#5EC98A" : "#E87040", fontSize: 10 }}>
                {diff >= 0 ? "+" : ""}{(spyLast).toFixed(2)}%
              </span>
            </div>
          );
        })()}
        {purchaseMarkers.map(m => (
          <div key={m.ticker + m.date} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "#444" }}>
            ...
            <span style={{ color: "#333" }}>{m.beforeRange ? "(prima del periodo)" : `@ $${fmt(m.pct)}`}</span>
          </div>
        ))}
      </div>
    </div>
