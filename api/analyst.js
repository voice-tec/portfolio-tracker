...
export default async function handler(req, res) {
  ...
  if (req.query.earnings === "true") {
    try {
      ...
      if (!er.ok) return res.status(404).json({ error: "No earnings data" });
      const edata = await er.json();
      const eresult = edata?.quoteSummary?.result?.[0];
      if (!eresult) return res.status(404).json({ error: "No result" });

      const earningsHistory = eresult.earningsHistory?.history || [];
      const nextEarnings = eresult.calendarEvents?.earnings?.earningsDate?.[0]?.raw;

      const now3y = Math.floor(Date.now() / 1000) - 3 * 365 * 86400;
      const priceUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(s)}?interval=1d&period1=${now3y}&period2=${Math.floor(Date.now()/1000)}`;
      let priceMap = {};
      try {
        const pr = await fetch(priceUrl, { headers: { "User-Agent": "Mozilla/5.0" } });
        if (pr.ok) {
          ...
          if (presult) {
            ...
            ts.forEach((t, i) => { if (closes[i] != null) priceMap[new Date(t*1000).toISOString().slice(0,10)] = closes[i]; });
          }
        }
      } catch {}
      ...
      const getPriceNear = (dateStr, offsetDays) => {
        ...
        for (let delta = 0; delta <= 5; delta++) {
          for (const sign of [1, -1]) {
            const try1 = new Date(d); try1.setDate(d.getDate() + offsetDays + sign * delta);
            const k = try1.toISOString().slice(0, 10);
      ...
      const earnings = earningsHistory.filter(e => e.quarter?.raw).map(e => {
        ...
        const moveDay = priceBefore && priceDay ? parseFloat(((priceDay - priceBefore) / priceBefore * 100).toFixed(2)) : null;
        const move2w  = priceBefore && priceAfter2w ? parseFloat(((priceAfter2w - priceBefore) / priceBefore * 100).toFixed(2)) : null;
        const qDate = new Date(e.quarter.raw * 1000);
        const qNum  = Math.floor(qDate.getMonth() / 3) + 1;
        return { date: dateStr, quarter: `Q${qNum} ${qDate.getFullYear()}`, epsActual, epsEstimate, surprise, beat: surprise != null ? surprise > 0 : null, moveDay, move2w };
      }).filter(e => e.epsActual !== null).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 12);

      const beats = earnings.filter(e => e.beat === true);
      const misses = earnings.filter(e => e.beat === false);
      const avgMoveOnBeat = beats.filter(e => e.moveDay !== null).length ? parseFloat((beats.filter(e => e.moveDay !== null).reduce((s,e) => s+e.moveDay, 0) / beats.filter(e => e.moveDay !== null).length).toFixed(2)) : null;
      const avgMoveOnMiss = misses.filter(e => e.moveDay !== null).length ? parseFloat((misses.filter(e => e.moveDay !== null).reduce((s,e) => s+e.moveDay, 0) / misses.filter(e => e.moveDay !== null).length).toFixed(2)) : null;

      return res.status(200).json({
        symbol: s, nextEarnings: nextEarnings ? new Date(nextEarnings * 1000).toISOString().slice(0, 10) : null,
        earnings,
        stats: { totalReported: earnings.length, beatRate: earnings.length ? Math.round(beats.length / earnings.length * 100) : null, avgMoveOnBeat, avgMoveOnMiss },
      });
    } catch (err) {
